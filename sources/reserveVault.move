module publisher::reserve_vault {
    ...
    fun init_client(sig: &signer) {
        ...
        let weightLocation = WeightLocation { g28: loc_map, g57: loc_map, g114: loc_map };
        let vault = Vault { gold: weightLocation, silver: weightLocation };
        move_to(sig, vault);
    }
    ...
    fun reserve_vault1(user1: signer, user2: signer) acquires Vault {
        ...
        init_client(&user2);
        assert!(
            exists<Vault>(signer::address_of(&user1)) == true,
            101
        );
        assert!(
            exists<Vault>(signer::address_of(&user2)) == true,
            101
        );
