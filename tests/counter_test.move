...
module publisher::counter_tests {
    ...
    public entry fun test_if_it_init() {
        ...
        print(&mesg);
        assert!(counter::get_count(addr) == 0, DNOT_EXIST);
        assert!(counter::get_mesg(addr) == utf8(b""), DNOT_EXIST);

        counter::increase(&account);
        assert!(counter::get_count(addr) == 1, DNOT_INCREASE);
        assert!(counter::get_mesg(addr) == utf8(b"one"), DNOT_EXIST);
    }
